...
import { stripe, PLANS, getPlanByPriceId, LIFETIME_PRICE_ID, LIFETIME_PRICE } from "@/lib/stripe/config";
...
import {
  createShareEmailCheckoutRefId,
  ensureShareEmailCheckoutRefsReady,
...
export async function POST(request: Request) {
  try {
    ...
    if (purchaseType === "email_share_batch") {
      ...
      await ensureShareEmailCheckoutRefsReady();
      const refId = createShareEmailCheckoutRefId();
    ...
    const checkoutParams: Stripe.Checkout.SessionCreateParams = {
      ...
      subscription_data: {
        metadata: { userId: session.user.email, planType, purchaseType: "subscription" },
      },
    };
