...
export default function ImageLibraryPanel({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-[#a39a88] p-6">
      <h2 className="text-lg font-bold text-[#33312e] mb-4 flex items-center gap-2">
        <span className="w-8 h-8 rounded-lg bg-[#7c5cff]/10 text-[#7c5cff] flex items-center justify-center">
        ...
        Image Library
        {!isPremium && (
          <span className="ml-auto text-[10px] font-bold text-[#ffb800] bg-[#ffb800]/10 px-2 py-0.5 rounded-full">PREMIUM</span>
        )}
      </h2>

      {!isPremium ? (
        <div className="text-center py-6">
          <div className="w-14 h-14 mx-auto mb-3 bg-[#7c5cff]/10 rounded-full flex items-center justify-center">
            <svg className="w-7 h-7 text-[#7c5cff]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
          </div>
          <h3 className="text-sm font-bold text-[#33312e] mb-1">Picture Bingo is Premium</h3>
          <p className="text-xs text-[#6b6459] mb-4 max-w-[220px] mx-auto">
            Add images to your bingo cells, Loteria-style. Upload your own or use our clip-art library.
          </p>
          <a
            href="/pricing"
            className="inline-block px-5 py-2.5 bg-[#7c5cff] text-white text-sm font-bold rounded-xl hover:shadow-md transition-all"
          >
            Subscribe for $7.99/mo
          </a>
          <p className="mt-2 text-[10px] text-[#6b6459]">Cancel anytime</p>
        </div>
      ) : selectedCellIndex !== null ? (
        <p className="text-xs text-[#7c5cff] font-medium mb-3">
      ...
      {isPremium && (<>
      {/* Tabs */}
      <div className="flex gap-1 bg-[#fff7ed] rounded-lg p-0.5 mb-4">
        <button
          onClick={() => setTab("my")}
          className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-all ${
            tab === "my"
              ? "bg-white text-[#33312e] shadow-sm"
      ...
      {tab === "my" && (
        <div className="mb-4">
          {usage.canUpload ? (
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-all ${
                uploading
                  ? "border-[#a39a88] bg-[#fff7ed]"
                  : "border-[#a39a88] hover:border-[#7c5cff] hover:bg-[#7c5cff]/30"
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              {uploading ? (
                ...
                <>
                  <svg className="w-6 h-6 mx-auto mb-1 text-[#6b6459]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4v16m8-8H4" />
                  </svg>
                  <p className="text-xs text-[#6b6459]">
                    Drop image or click to upload
                  </p>
                  <p className="text-[10px] text-[#6b6459] mt-1">
            ...
            <div className="border border-[#ffb800] bg-[#ffb800]/10 rounded-xl p-3 text-center">
              <p className="text-xs text-[#ffb800] font-medium">
                Custom image uploads are not available on this plan
              </p>
              <a
                href="/pricing"
                className="text-[10px] text-[#7c5cff] font-semibold underline mt-1 inline-block"
              >
                See plan options
              </a>
            </div>
          ...
          {uploadError && (
            <p className="mt-2 text-xs text-[#ff5d8f]">{uploadError}</p>
          )}
        </div>
      )}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-8">
          <div className="w-5 h-5 border-2 border-[#a39a88] border-t-[#7c5cff] rounded-full animate-spin" />
        </div>
      ) : images.length === 0 ? (
        <p className="text-xs text-[#6b6459] text-center py-6">
          ...
          {images.map((img) => (
            <button
              ...
              className={`relative aspect-square rounded-lg border overflow-hidden transition-all group ${
                selectedCellIndex !== null
                  ? "hover:ring-2 hover:ring-[#7c5cff] cursor-pointer border-[#a39a88]"
                  : "border-[#fff7ed] opacity-60 cursor-not-allowed"
              }`}
            >
              <img
              ...
              {tab === "my" && "size" in img && (
                <button
                  ...
                  className="absolute top-0.5 right-0.5 w-4 h-4 bg-[#ff5d8f] text-white rounded-full text-[8px] font-bold hidden group-hover:flex items-center justify-center"
                >
                  X
                </button>
              )}
