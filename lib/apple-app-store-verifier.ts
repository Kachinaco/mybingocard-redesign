import {
  Environment,
  SignedDataVerifier,
  type JWSTransactionDecodedPayload,
  type JWSRenewalInfoDecodedPayload,
  type ResponseBodyV2DecodedPayload,
} from "@apple/app-store-server-library";

export const APPLE_BUNDLE_ID = "com.coryanalla.MyBingoCardApp";
export const APPLE_APP_ID = 6767310336;

// DER-encoded Apple roots published at https://www.apple.com/certificateauthority/.
// Keeping the trust anchors in source makes verification deterministic in Next.js
// bundles and prevents a production host's OS trust store from changing behavior.
const APPLE_ROOT_CERTIFICATES = [
  "",
  "",
  "",
].map((certificate) => Buffer.from(certificate, "base64"));

function decodePayload<T>(jws: string): T {
  const parts = jws.split(".");
  if (parts.length !== 3) throw new Error("Invalid Apple signed payload.");
  return JSON.parse(Buffer.from(parts[1]!, "base64url").toString("utf8")) as T;
}

function verifierForEnvironment(environment: Environment): SignedDataVerifier {
  const enableOnlineChecks = process.env.APPLE_IAP_ENABLE_ONLINE_CHECKS !== "0";
  return new SignedDataVerifier(
    APPLE_ROOT_CERTIFICATES,
    enableOnlineChecks,
    environment,
    APPLE_BUNDLE_ID,
    environment === Environment.PRODUCTION ? APPLE_APP_ID : undefined,
  );
}

function supportedEnvironment(value: unknown): Environment.SANDBOX | Environment.PRODUCTION {
  if (value === Environment.SANDBOX) return Environment.SANDBOX;
  if (value === Environment.PRODUCTION) return Environment.PRODUCTION;
  throw new Error("Unsupported Apple transaction environment.");
}

export async function verifyAndDecodeAppleTransaction(
  signedTransactionInfo: string,
): Promise<JWSTransactionDecodedPayload> {
  const unverified = decodePayload<{ environment?: string }>(signedTransactionInfo);
  const environment = supportedEnvironment(unverified.environment);
  return verifierForEnvironment(environment).verifyAndDecodeTransaction(signedTransactionInfo);
}

export async function verifyAndDecodeAppleNotification(
  signedPayload: string,
): Promise<ResponseBodyV2DecodedPayload> {
  const unverified = decodePayload<{
    data?: { environment?: string };
    summary?: { environment?: string };
    appData?: { environment?: string };
    externalPurchaseToken?: { externalPurchaseId?: string };
  }>(signedPayload);
  const environmentValue = unverified.data?.environment
    || unverified.summary?.environment
    || unverified.appData?.environment
    || (unverified.externalPurchaseToken?.externalPurchaseId?.startsWith("SANDBOX")
      ? Environment.SANDBOX
      : unverified.externalPurchaseToken
        ? Environment.PRODUCTION
        : undefined);
  const environment = supportedEnvironment(environmentValue);
  return verifierForEnvironment(environment).verifyAndDecodeNotification(signedPayload);
}

export async function verifyAndDecodeAppleRenewalInfo(
  signedRenewalInfo: string,
): Promise<JWSRenewalInfoDecodedPayload> {
  const unverified = decodePayload<{ environment?: string }>(signedRenewalInfo);
  const environment = supportedEnvironment(unverified.environment);
  return verifierForEnvironment(environment).verifyAndDecodeRenewalInfo(signedRenewalInfo);
}
